...
class Projects_model extends CI_Model
{
     ...
     * @return    array
     */
    public function get_projects_by_map($id)
    {

        $this->db
			->select('p.pid, p.slug, p.uid, p.fundamental_legal, p.stage, p.sector, p.country, projectname, last_date, CASE WHEN fp.project_id IS NULL THEN 0 ELSE 1 END AS selected', false)
			->from('exp_projects p')
...
*/
	 ...
	 * @return 	array
	*/
    public function get_proj_energy_data($slug)
    {
        $qryproj = $this->db->get_where("exp_proj_ext_energy",array("slug"=>$slug));
        $projectarr = $qryproj->row_array();
        $qryproj->free_result();
     ...
     * @return 	array
    */
    public function get_proj_transport_data($slug)
    {
        $qryproj = $this->db->get_where("exp_proj_ext_transport",array("slug"=>$slug));
        $projectarr = $qryproj->row_array();
        $qryproj->free_result();
     ...
     * @return 	array
    */
    public function get_proj_water_data($slug)
    {
        $qryproj = $this->db->get_where("exp_proj_ext_water",array("slug"=>$slug));
        $projectarr = $qryproj->row_array();
        $qryproj->free_result();
     ...
     * @return 	array
    */
    public function get_proj_IT_data($slug)
    {
        $qryproj = $this->db->get_where("exp_proj_ext_IT",array("slug"=>$slug));
        $projectarr = $qryproj->row_array();
        $qryproj->free_result();

...
*/
    ...
    public function books(){
        ...
        $book_data	=   $this->profile_model->get_books($this->sess_uid);
        ...
        // Render HTML Page from view direcotry
        $this->load->view('templates/header', $this->headerdata);
        $this->load->view('profile/books', $data);
        $this->load->view('templates/footer', $this->dataLang);
    ...
    public function create_book(){
        ...
        $this->form_validation->set_rules('book_content', 'Book Content', 'required');
        if ($this->form_validation->run() === FALSE)
        {
            ...
            $response = array();
            $response["status"] 	= "error";
            $response["message"] 	= array('book_title'=>form_error('book_title'),
                'book_content'=>form_error('book_content')
            );
            $response["remove"] 	= true;
            $response["isload"] 	= "no";
            //$response["loadurl"] 	= "/projects/form_load/expertise_sector_form/view/";
        ...
        {
            $this->profile_model->create_book();
        }
    ...
    public function delete_book($id){
        $this->profile_model->delete_book($id);
    }
    ...
    public function update_book($id){
        ...
        //define validation rules
        $this->form_validation->set_error_delimiters('<label>', '</label>');
        $this->form_validation->set_rules('book_title', 'Book Name', 'required');
        $this->form_validation->set_rules('book_content', 'Book Content', 'required');
        if ($this->form_validation->run() === FALSE)
        {
            //ajax response for error massages
        ...
        {
            $this->profile_model->edit_book($id);
        }
    }

    public function upload_bookphoto($id)
    {
        $book_id = $id;
        ...
        $error_delimiters = array('open' => '<label>', 'close' => '</label>');
        $upload = upload_image(BOOK_IMAGE_PATH, 'photo_filename', true, array(), $error_delimiters);
        ...
        if (! $this->profile_model->update_photo($book_id, $upload['file_name'])) {
            $error_response['message'] = 'error uploading book image';
            sendResponse($error_response);
            exit;
        }
        ...
        $response = array(
            ...
            'message' => lang('Profilepictureupdatedsuccessfully'),
            'isload' => 'no',
            'imgpath' => book_image($upload['file_name'], 150),
            'headerimgpath' => book_image($upload['file_name'], 27, array('rounded_corners' => array('all', '2'))),
        );

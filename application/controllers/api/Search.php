...
//				}
    ...
    private function search_newsfeed($filters, $limit = 10)
    {
        $newsfeed = array();

        $newsfeed_filters = $filters;
        $newsfeed_options = array(
            'gviptv_filter' => $this->input->get_post('country_filter'),
            'projects_stage_filter'	=> $this->input->get_post('projects_stage_filter'),
            'projects_sector_filter'	=> $this->input->get_post('projects_sector_filter'),
            'projects_value_filter'	=> $this->input->get_post('projects_value_filter'),
            'experts_discipline_filter' => $this->input->get_post('experts_discipline_filter'),
            'experts_sector_filter' => $this->input->get_post('experts_discipline_filter'),
        );
...
//		die_json($response);
    ...
    public function geocode_google()
    {
        // If the user is not logged in then redirect to the login page
        auth_check();
        ...
        $response = $this->googlegeocoder->geocode(urlencode($address));
        //$response = json_decode($response);
        die_json($response);
    }

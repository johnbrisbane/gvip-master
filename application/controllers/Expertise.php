...
class Expertise extends CI_Controller {
    ...
    public function view($userid)
	{
	    ...
	    $project = $this->expertise_model->get_organization_projects_data($userid);
            ...
            elseif ($userid == 4653){
                $view = 'expertise/organization_view_airgus';
            }
	        else{
		...
		$books          =   $this->expertise_model->get_books($userid);
    ...
    public function biden_admin_dep()
    {
        // If the current user doesn't have access to the module show 404
        ...
        $file = fopen('BidenAdminforGVIP.csv', 'r');
        $all_rows = array();
        $header = fgetcsv($file);
        ...
        // Render the page
        $this->load->view('templates/header', $this->headerdata);
        $this->load->view('expertise/biden_admin_dep', $data);
        $this->load->view('templates/footer', $this->dataLang);
    ...
    public function biden_admin($dep){
        ...
        // Render the page
        $this->load->view('templates/header', $this->headerdata);
        $this->load->view('expertise/biden_admin', $data);
        $this->load->view('templates/footer', $this->dataLang);
    ...
    public function book_view($id){
        ...
        $specificbook = $this->expertise_model->get_books_specific($id);

        $book_data	= $this->profile_model->get_books($specificbook->member_id);

        $created_at = DateFormat($specificbook->created_at, DATEFORMAT, FALSE);
        $updated_at = DateFormat($specificbook->updated_at, DATEFORMAT, FALSE);

        $totalbooks = count($book_data);
        ...
        $this->load->view('templates/header', $this->headerdata);
        $this->load->view('expertise/book_view', $data);
        $this->load->view('templates/footer', $this->dataLang);

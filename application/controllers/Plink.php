...
class Plink extends CI_Controller
{
    ...
    protected function email_member_med($id) {
        $this->load->model('expertise_model');
        ...
        if ($member['membertype'] == MEMBER_TYPE_MEMBER) {
            $src = expert_image($member['userphoto'], 130, array('rounded_corners' => array('all', 0)));
        } else {
            $src = company_image($member['userphoto'], 130, array('rounded_corners' => array('all', 0)));
        }

    ...
    */
	...
	$viewdata['pci'] = 0;
        ...
        if (! ($viewdata['project']['files']['totalfiles'] == 0)) {
            ...
            $viewdata['project_sections']['timeline'] = true;
    ...
    */
    public function edit($params)
    {
        ...
        if ($this->input->post('return') != '') {
            ...
            $this->form_validation->set_rules('project_conceptual', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period_conceptual');
            $this->form_validation->set_rules('project_dates_feasibility', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period');
            $this->form_validation->set_rules('project_dates_planning', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period');

            $this->form_validation->set_rules('project_conceptual_to', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period_conceptual');
            $this->form_validation->set_rules('project_dates_feasibility_to', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period');
            $this->form_validation->set_rules('project_dates_planning_to', lang('valid_period'), 'trim|callback_valid_monthyear_format|callback_valid_period');

            $this->form_validation->set_rules('people_served', 'People Served', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('property_val', 'Increased Property Value ($MM)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('marg_peopleserved', 'Marginal People Served', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('co2_saved', 'CO2 Emissions Saved (Tons / Year)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('econ_impact', 'Economic Impact ($MM / Year)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('oil_eliminated', 'Fossil Fuels Eliminated (Tons / Year)', 'integer|greater_than[-1]');

            //Energy
            $this->form_validation->set_rules('generation', 'Generation (MegaWatts)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('storage', 'Storage (MegaWatts Hours)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('reduced_water', 'Reduced Water Use (Millions of Gallons)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('trans_dist', 'Transmission Distance (Miles)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('trans_cap', 'Transmission Delivery Capacity (MegaWatts)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('hydrogen_tons', 'Hydrogen Produced (Metric Tons / Day)', 'integer|greater_than[-1]');

            //Transport
            $this->form_validation->set_rules('miles', 'Distance (miles)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('add_riders', 'Additional Daily Riders:', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('add_goods', 'Additional Daily Goods (Metric Tons / Day)', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('vmt', 'Automobile Vehicle Miles Traveled', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('avg_ridership', 'Average Weekday Ridership', 'integer|greater_than[-1]');
            $this->form_validation->set_rules('capacity_added', 'Capacity Added (TEU\'s)', 'integer|greater_than[-1]');

            //IT
            $this->form_validation->set_rules('mb_per_sec', 'Megabytes Per Second)', 'integer|greater_than[-1]');

            //Water
            $this->form_validation->set_rules('total_capacity', 'Total Capacity (Millions of Gallons / Day))', 'integer|greater_than[-1]');
        ...
        $editdata['project']['energy'] = $this->projects_model->get_proj_energy_data($slug,$this->uid);
        $editdata['project']['water'] = $this->projects_model->get_proj_water_data($slug,$this->uid);
        $editdata['project']['transport'] = $this->projects_model->get_proj_transport_data($slug,$this->uid);
        $editdata['project']['IT'] = $this->projects_model->get_proj_IT_data($slug,$this->uid);
        ...
        $editdata['project']['conceptual_date_from'] = $this->cleanDate($editdata['project']['conceptual_date_from']);
        $editdata['project']['feasibility_date_from'] = $this->cleanDate($editdata['project']['feasibility_date_from']);
        $editdata['project']['planning_date_from'] = $this->cleanDate($editdata['project']['planning_date_from']);

        $editdata['project']['conceptual_date_to'] = $this->cleanDate($editdata['project']['conceptual_date_to']);
        $editdata['project']['feasibility_date_to'] = $this->cleanDate($editdata['project']['feasibility_date_to']);
        $editdata['project']['planning_date_to'] = $this->cleanDate($editdata['project']['planning_date_to']);
...
//                'project_design_issues_attachment' => form_error('project_design_issues_attachment')
    ...
    */
    ...
    public function valid_period_conceptual()
    {
        $start = $this->input->post('project_conceptual', true);
        $end = $this->input->post('project_conceptual_to', true);

        $is_valid = is_valid_period($start, $end, 'm/Y');

        return $is_valid;
    }
    public function valid_period_feasibility()
    {
        $start = $this->input->post('project_feasibility', true);
        $end = $this->input->post('project_feasibility_to', true);

        $is_valid = is_valid_period($start, $end, 'm/Y');

        return $is_valid;
    }
    public function valid_period_planning()
    {
        $start = $this->input->post('project_planning', true);
        $end = $this->input->post('project_planning_to', true);

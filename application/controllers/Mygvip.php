...
class Mygvip extends CI_Controller {
    ...
    public function index()
    {
        ...
        if (empty($key_executives)) {
            $data = $this->expertise_model->get_filter_user_list2(3, 0, null, 5, 5);
        ...
        $gviptvrows = $this->newsfeed_model->all();

        $books = $this->newsfeed_model->all_books();

        $sort = 2;

        $projects = $this->newsfeed_model->allprojets(50, NULL, NULL, $sort);
        $users = $this->newsfeed_model->get_filter_user_list2(25);
        $users = $users['filter'];

        $model_obj = $this->newsfeed_model;
...
//        }
        ...
        $this->load->view('templates/_map_assets', '');
        $this->load->view('templates/_map_templates', '');
        $this->load->view('mygvip/map', $data);
        $this->load->view('templates/footer', $this->footer_data);

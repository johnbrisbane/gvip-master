...
//        "
        ...
        $similar = $this->db
            ->select('pid id, projectname, projectphoto, sector, stage, totalbudget, slug')
            ->from('exp_projects')
            ->where_in('pid', $sp_ids)
            ->get()
            ->result_array();
    ...
    public function update_proj_geocode_from_location($slug="",$uid="",$location="",$country="")
    {
        ...
        foreach($qry->result() as $i => $row )
        {
            ...
            $data = $this->googlegeocoder->geocode($location);

            // create insert obj
            $insert_data = array();
            $insert_data['geocode'] = json_encode($data->results[0]);

            if($data && count($data->results) > 0 && $data->results[0]->formatted_address != "" )
            {
                //echo "<pre>"; var_dump( $json ); exit;
                $loc1 = $data->results[0]->geometry->location;
                $insert_data['lat'] = $loc1->lat;
                $insert_data['lng'] = $loc1->lng;
	...
	public function get_city_state($slug,$uid)
	{
		...
		if (array_key_exists('formatted_address', $geocode)) {
			$prettylocation = $geocode['formatted_address'];
		}

		if (array_key_exists('results', $geocode)) {
			if (count($geocode['results'][0]['locations'])) {
    ...
    */
	public function update_project($slug, $uid)
	{
		...
		$estconceptual = $this->input->post('project_conceptual', TRUE);
		$estfeasibility = $this->input->post('project_feasibility', TRUE);
		$estplanning = $this->input->post('project_planning', TRUE);
		$estconceptualto = $this->input->post('project_conceptual_to', TRUE);
		$estfeasibilityto = $this->input->post('project_feasibility_to', TRUE);
		$estplanningto = $this->input->post('project_planning_to', TRUE);
		...
		if ($estconceptual != "1111-11-11 00:00:00" && $estconceptual != "" ) {
			if (strlen($estconceptual) == 7) {
				$estconceptual = substr_replace($estconceptual, "01/", 3, 0);
			}
		} else {
			$estconceptual = "";
		}
		if ($estfeasibility != "1111-11-11 00:00:00" && $estfeasibility != "" ) {
			if (strlen($estfeasibility) == 7) {
				$estfeasibility = substr_replace($estfeasibility, "01/", 3, 0);
			}
		} else {
			$estfeasibility = "";
		}

		if ($estplanning != "1111-11-11 00:00:00" && $estplanning != "" ) {
			if (strlen($estplanning) == 7) {
				$estplanning = substr_replace($estplanning, "01/", 3, 0);
			}
		} else {
			$estplanning = "";
		}

		if ($estconceptualto != "1111-11-11 00:00:00" && $estconceptualto != "" ) {
			if (strlen($estconceptualto) == 7) {
				$estconceptualto = substr_replace($estconceptualto, "01/", 3, 0);
			}
		} else {
			$estconceptualto = "";
		}
		if ($estfeasibilityto != "1111-11-11 00:00:00" && $estfeasibilityto != "" ) {
			if (strlen($estfeasibilityto) == 7) {
				$estfeasibilityto = substr_replace($estfeasibilityto, "01/", 3, 0);
			}
		} else {
			$estfeasibilityto = "";
		}

		if ($estplanningto != "1111-11-11 00:00:00" && $estplanningto != "" ) {
			if (strlen($estplanningto) == 7) {
				$estplanningto = substr_replace($estplanningto, "01/", 3, 0);
			}
		} else {
			$estplanningto = "";
		}
		...
		//convert array of values to null from 0 or ''
		$numerics = array(
			'people_served',
			'property_val',
			'marg_peopleserved',
        ...
        foreach ($numerics as $numeric){
        	$captureddata = $this->input->post($numeric,TRUE);
        	if ($captureddata == ''){
        		$converteddata[$numeric] = null;
			}
        	else{
				$converteddata[$numeric] = (int) $captureddata;
			}
		}
		...
		$update = array(
			...
			'conceptual_date_from'	=> DateFormat($estconceptual, DATEFORMATDB, FALSE),
			'feasibility_date_from' => DateFormat($estfeasibility, DATEFORMATDB, FALSE),
			'planning_date_from'	=> DateFormat($estplanning, DATEFORMATDB, FALSE),
			'conceptual_date_to'	=> DateFormat($estconceptualto, DATEFORMATDB, FALSE),
			'feasibility_date_to' 	=> DateFormat($estfeasibilityto, DATEFORMATDB, FALSE),
			'planning_date_to'		=> DateFormat($estplanningto, DATEFORMATDB, FALSE),
			...
			'people_served'		    => $converteddata['people_served'],
			'property_val'		    => $converteddata['property_val'],
			'marg_peopleserved'		=> $converteddata['marg_peopleserved'],
			'co2_saved'		        => $converteddata['co2_saved'],
			'econ_impact'		    => $converteddata['econ_impact'],
			'oil_eliminated'		=> $converteddata['oil_eliminated'],
		);
		...
		//Energy Specific
        if ($update['sector'] == 'Energy'){
			$energy_update = array (
				'slug'				=> $slug,
				'uid'				=> $uid,
				'generation'		=> $converteddata['people_served'],
				'storage'			=> $converteddata['storage'],
				'trans_dist'		=> $converteddata['trans_dist'],
				'trans_cap'			=> $converteddata['trans_cap'],
				'reduced_water'		=> $converteddata['reduced_water'],
				'hydrogen_tons'		=> $converteddata['hydrogen_tons'],
			);


			$query = null;
			$query = $this->db->get_where('exp_proj_ext_energy',array('slug' => $slug, 'uid' => $uid));
			$count = $query->num_rows(); //counting result from query

			if ($count === 0) {
				$this->db
					->insert('exp_proj_ext_energy', $energy_update);
			}
			else {
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->update('exp_proj_ext_energy', $energy_update);
			}

			if ($this->db->get_where('exp_proj_ext_IT',array('slug' => $slug, 'uid' => $uid))->num_rows() > 0){
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->delete('exp_proj_ext_IT');
			}
			if ($this->db->get_where('exp_proj_ext_water',array('slug' => $slug, 'uid' => $uid))->num_rows() > 0){
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->delete('exp_proj_ext_water');
			}
			if ($this->db->get_where('exp_proj_ext_transport',array('slug' => $slug, 'uid' => $uid))->num_rows() > 0){
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->delete('exp_proj_ext_transport');
			}
		}
		...
		if ($update['sector'] == 'Water'){

			$water_update = array (
				'slug'				=> $slug,
				'uid'				=> $uid,
				'total_capacity'	=> $converteddata['total_capacity'],
			);

			$query = null;
			$query = $this->db->get_where('exp_proj_ext_water',array('slug' => $slug, 'uid' => $uid));
			$count = $query->num_rows(); //counting result from query

			if ($count === 0) {
				$this->db
					->insert('exp_proj_ext_water', $water_update);
			}
			else {
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->update('exp_proj_ext_water', $water_update);
			}

			if ($this->db->get_where('exp_proj_ext_energy',array('slug' => $slug, 'uid' => $uid))->num_rows() > 0){
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->delete('exp_proj_ext_energy');
			}
			if ($this->db->get_where('exp_proj_ext_IT',array('slug' => $slug, 'uid' => $uid))->num_rows() > 0){
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
		...
		//Transport Specific
		if ($update['sector'] == 'Transport'){

			$transport_update = array (
				'slug'				=> $slug,
				'uid'				=> $uid,
				'miles'					=> $converteddata['miles'],
				'add_riders'			=> $converteddata['add_riders'],
				'add_goods'				=> $converteddata['add_goods'],
				'vmt'					=> $converteddata['vmt'],
				'avg_ridership'			=> $converteddata['avg_ridership'],
				'capacity_added'		=> $converteddata['capacity_added'],
			);
			$query = null;
			$query = $this->db->get_where('exp_proj_ext_transport',array('slug' => $slug, 'uid' => $uid));
			$count = $query->num_rows(); //counting result from query

			if ($count === 0) {
				$this->db
					->insert('exp_proj_ext_transport', $transport_update);
			}
			else {
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->update('exp_proj_ext_transport', $transport_update);
			}
		...
		//IT Specific
		if ($update['sector'] == 'Information & Communication Technologies'){

			$IT_update = array (
				'slug'				=> $slug,
				'uid'				=> $uid,
				'mb_per_sec'		=> $converteddata['mb_per_sec'],
			);
			$query = null;
			$query = $this->db->get_where('exp_proj_ext_IT',array('slug' => $slug, 'uid' => $uid));
			$count = $query->num_rows(); //counting result from query

			if ($count === 0) {
				$this->db
					->insert('exp_proj_ext_IT', $IT_update);
			}
			else {
				$this->db
					->where(array('slug' => $slug, 'uid' => $uid))
					->update('exp_proj_ext_IT', $IT_update);
			}
	...
	*/
	...
	/*
	 * Get Project Extension Energy Data
	 * Get Requested Project data when user eneter into edit mode
	 *
	 * @access 	public
	 * @param 	string
	 ...
	 * @return 	array
	*/
	public function get_proj_energy_data($slug,$uid)
	{
		$qryproj = $this->db->get_where("exp_proj_ext_energy",array("slug"=>$slug,"uid"=>$uid));
		$projectarr = $qryproj->row_array();
		$qryproj->free_result();
	...
	/*
	 * Get Project Extension Transport Data
	 * Get Requested Project data when user eneter into edit mode
	 *
	 * @access 	public
	 * @param 	string
	 ...
	 * @return 	array
	*/
	public function get_proj_transport_data($slug,$uid)
	{
		$qryproj = $this->db->get_where("exp_proj_ext_transport",array("slug"=>$slug,"uid"=>$uid));
		$projectarr = $qryproj->row_array();
		$qryproj->free_result();
	...
	/*
	 * Get Project Extension Water Data
	 * Get Requested Project data when user eneter into edit mode
	 *
	 * @access 	public
	 * @param 	string
	 ...
	 * @return 	array
	*/
	public function get_proj_water_data($slug,$uid)
	{
		$qryproj = $this->db->get_where("exp_proj_ext_water",array("slug"=>$slug,"uid"=>$uid));
		$projectarr = $qryproj->row_array();
		$qryproj->free_result();
	 ...
	 * @return 	array
	*/
	public function get_proj_IT_data($slug,$uid)
	{
		$qryproj = $this->db->get_where("exp_proj_ext_IT",array("slug"=>$slug,"uid"=>$uid));
		$projectarr = $qryproj->row_array();
		$qryproj->free_result();
	...
	*/
	...
	public function gget_proj_map_data()
	{

	...
	*/
    ...
    public function create_book(){
        ...
        $linkdata = array(
            ...
            'content' => $this->input->post('book_content'),
            'status'	  => '1',
            'member_id'		  => $userid,
            'updated_at' => $now
        );
        ...
        //insert into db and set session
        if($this->db->insert('exp_library', $linkdata))
        {
            $response = array();
            $response["status"] 		= "success";
    ...
    public function edit_book($id){
        ...
        //create profile information post array
        $input = array(
            'title'           => $this->input->post('book_title', TRUE),
            'content'            => $this->input->post('book_content', TRUE),
            'updated_at'      => $now
        );
        ...
        if ($str = $this->db->update('exp_library', $input)) {
    ...
    public function delete_book($id){
        ...
        if($str = $this->db->delete('exp_library', array('id'=>$id)))
        {
            $response["remove"]	= TRUE;
            //header('Content-type: application/json');
            echo json_encode($response);
    ...
    public function get_books($uid){

        $qryexec = $this->db->get_where("exp_library",array("member_id"=>$uid));
    ...
    public function update_bookphoto($id, $filename)
    {
        $book_id = (int) $id;
        ...
        if (! $this->db->update('exp_library', $data)) {
            return false;
        }

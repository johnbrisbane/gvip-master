...
class Migration_add_smartcities_subsector extends CI_Migration {
    ...
    private $with = "
        ...
        (
            SELECT sectorid
              FROM exp_sectors
             WHERE sectorname = 'Smart Cities'
               AND parentid = 0
             LIMIT 1 -- Just for a good measure to ensure the only record
        ), new_subsectors AS
        (
            SELECT 'Mobility'::VARCHAR sectorvalue
            'Environment'::VARCHAR sectorvalue
        )   
        ";
